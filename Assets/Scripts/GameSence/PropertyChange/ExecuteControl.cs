...
namespace GameSence.PropertyChange
{
    ...
    public class ExecuteControl : MonoBehaviour
    {
        ...
        //本周教室的日程安排
        private Unit.Schedule[] classroomSchedule;
        ...
        public void Init()
        {
            ...
            courseList = GameManager.GameManager.Instance.CourseList;
            studentUnits = GameManager.GameManager.Instance.saveObject.SaveData.studentUnits;
            classroomSchedule = GameManager.GameManager.Instance.saveObject.SaveData.classroomSchedule;
            playerCourses = GameManager.GameManager.Instance.saveObject.SaveData.playerUnit.playerCourses;
            playerCourseLevelList = GameManager.GameManager.Instance.PlayerCourseLevelList;
        ...
        public void LoadStudent(StudentUnit _studentUnit)
        {
            ...
            //这里固定肖像
            foreach (var obj in studentsPortrayals) obj.SetActive(obj.CompareTag(_studentUnit.id));
        ...
        private void StudentAttributeUpdate()
        {
            ...
            {
                var row = courseList.Find_Id(schedule.id);
                rows.Add(row);
            }
            ...
            //复制一个学生，用来对比属性的变化
            //这个是初始的学生数值
            var lowStudent = Copy(studentUnit);
            //这个是结算仅课程加成的学生数值
            for (var i = 0; i < rows.Count; i++)
            {
                ...
                if (classroomSchedule[i].id == rows[i].Id)
                    //此时当前课程是在教室上的
                    learningMultiple = GameManager.GameManager.Instance.saveObject.SaveData.classroomEfficiency;
            ...
            var classStudent = Copy(studentUnit); //class
            for (var i = 0; i < rows.Count; i++)
            {
                ...
                foreach (var playerCourse in playerCourses)
                {
                    ...
                    {
                        //找到符合该技能ID的升级数据
                        var allId = playerCourseLevelList.FindAll_id(playerCourse.id);
                        //找到符合该技能等级的升级数据
                        var allIdLevel =
                ...
                foreach (var target in targets)
                {
                    ...
                    {
                        target.语文, target.数学, target.英语, target.政治, target.历史, target.地理, target.物理, target.化学,
                        target.生物
                    };
                    ...
                    {
                        target.音乐, target.表演, target.舞蹈, target.手工, target.棋技, target.种植, target.摄影, target.烹饪,
                        target.考古,
            ...
            switch (location)
            {
                case Location.Low2New:
                {
                    ...
                    void Low2New(IReadOnlyList<Grade> newUnit, IReadOnlyList<Grade> lowUnit)
                    {
                        ...
                        {
                            var lowGrade = lowUnit[i];
                            var newGrade = newUnit[i];
