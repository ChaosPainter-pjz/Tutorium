...
using GameSence.PropertyChange;
using GameSence.StoryLine;
using GameSence.World.Game;
using Unit;
using UnityEngine;
...
namespace GameSence
{
    ...
    public class ExecuteManager : MonoInstance<ExecuteManager>
    {
        ...
        public void OnClick()
        {
            ...
            {
                SaveManager.Scripts.SaveManager.SaveGame(gameManager.saveObject.SaveData, 0);
                // SaveSchedule();
                //开始放动画片
                StopAllCoroutines();
            ...
            while (date.WhatDay != 0)
            {
                ...
                if (date.WhatDay != 0) //周日的剧情结算后触发，所以这里不能是周日
                    if (storyLineManager.JudgmentEnforceabilityRow())
                    {
                        dayAnimationControl.gameObject.SetActive(false);
                        storyLineManager.BeganPlot();
                        while (storyLineManager.isPlot) yield return null;
        ...
        private IEnumerator WorkDayAnimator()
        {
            ...
            while (date.WhatDay != 5)
            {
                ...
                DatetimeManager.Instance.DateTimeUpdate(1);
                yield return new WaitForSeconds(0.5f);
                if (storyLineManager.isEnd) yield break;
        ...
        private void StudentsIndicateUpdate()
        {
            foreach (var studentUnit in studentUnits)
            {
                foreach (var grade in studentUnit.indicatingNow) grade.score += 20;
        ...
        private void StudentWorkUpdate()
        {
            foreach (var studentUnit in studentUnits)
            {
                var findAllID = studentWeeklyPropertyList.FindAll_id(studentUnit.id);
                ...
                var row = findAllID[Random.Range(0, findAllID.Count)];
                string[] strings = { row.语文, row.数学, row.英语, row.政治, row.历史, row.地理, row.物理, row.化学, row.生物 };
                var ints = Array.ConvertAll(strings, int.Parse);
                for (var i = 0; i < studentUnit.mainGrade.Count; i++) studentUnit.mainGrade[i].score += ints[i];
            }
        }
        ...
        private void PlayerAttributeUpdate()
        {
            ...
            //遍历玩家的计划，变更教研、打工技能等级
            for (var i = 0; i < playerPlan.Length; i++)
            {
                //没有计划则跳过这次循环
                if (playerPlan[i] == "0") continue;
                var course = playerCourses.Find(x => x.id == playerPlan[i]);
                var coursesAllId = playerCourseLevelList.FindAll_id(playerPlan[i]);
                if (coursesAllId != null && coursesAllId.Count > 0) //现在确认id是教研表中的
                {
                    var nextLevelMaxXP = -1;
                    var maxLevel = 0;
                    foreach (var row in coursesAllId) //找到当前等级对应升级所需经验，最高等级
                    {
                        if (int.Parse(row.等级) == course.level) nextLevelMaxXP = int.Parse(row.下一级所需经验);

                        if (maxLevel < int.Parse(row.等级)) maxLevel = int.Parse(row.等级);
                    }
                ...
                {
                    var nextLevelMaxXP = int.Parse(workId.UpExperience);
                    var maxLevel = int.Parse(workId.maxLevel);
                ...
                */
                    var initialYield = int.Parse(workId.InitialYield);
                    var levelYield = int.Parse(workId.levelYield);
                    MoneyManager.Instance.Money += initialYield + course.level * levelYield;
                }
            }

            playerPropertyChangeControl.PlayerUpdatePanel();
            playerPropertyChangeControl.moneyControl.SetMoneyPanel(MoneyManager.Instance
                .Money); //传递新的金钱数,更新金钱词条的显示与否、位置
        }
    }
}