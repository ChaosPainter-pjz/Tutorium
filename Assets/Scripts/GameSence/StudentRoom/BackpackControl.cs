...
namespace GameSence.StudentRoom
{
    ...
    public class BackpackControl : MonoBehaviour
    {
        ...
        public void Init(StudentUnit studentUnit)
        {
            ...
            articleList ??= GameManager.GameManager.Instance.articleList;
            articles ??= GameManager.GameManager.Instance.saveObject.SaveData.playerUnit.articles;
        ...
        public void UpdateUI()
        {
            ...
            {
                var control = Instantiate(articleCard, articleParent).GetComponent<ArticleCard>();
            ...
            for (var i = 0; i < articles.Count; i++) articleCards[i].Init(articles[i]); //初始化卡片，里面带显示
        }
