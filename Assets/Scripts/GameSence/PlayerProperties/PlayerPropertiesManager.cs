...
namespace GameSence.PlayerProperties
{
    public class PlayerPropertiesManager : MonoInstance<PlayerPropertiesManager>
    {
        ...
        [Header("时间轴")] [SerializeField] private Toggle timerShaftButton;
        ...
        [Header("教师技能")] [SerializeField] private Transform playerSkillItemParent;
            ...
            foreach (var node in timerShaftNodes)
                switch (node.nodeType)
                {
                    case TimerShaftNode.NodeType.WhatDay:
                        var nodeObj = Instantiate(timerShaftNodeObject, timerShaftParent);
                        var control = nodeObj.GetComponent<TimerShaftNodeControl>();
                        control.Init(node, timerShaftStudentObject);
                        nodeList.Add(nodeObj);
                        break;
                    case TimerShaftNode.NodeType.Year:
                        var yearNodeObj = Instantiate(timerShaftYearObject, timerShaftParent);
        ...
        {
            var studentNode = new TimerShaftStudentNode(studentID, content);
            if (timerShaftNodes.Count == 0)
            {
                //增加年的节点
                timerShaftNodes.Add(new TimerShaftNode(TimerShaftNode.NodeType.Year,
                    new Unit.Date(DatetimeManager.Instance.InitYear, 0, 1, 0), null));
                var yearNodeObj = Instantiate(timerShaftYearObject, timerShaftParent);
                yearNodeObj.GetComponent<TimerShaftYearControl>().ThisDate = date.Copy();
                nodeList.Add(yearNodeObj);
            }

            var saveLowDate = timerShaftNodes[timerShaftNodes.Count - 1].date;

            //如果时间轴里最后一个普通节点的日期与当前时间相同，且是小节点
            if (saveLowDate == date &&
                timerShaftNodes[timerShaftNodes.Count - 1].nodeType == TimerShaftNode.NodeType.WhatDay)
            {
            ...
            else if (saveLowDate.Semester != date.Semester)
            {
                ...
                //增加普通节点
                var node = new TimerShaftNode(TimerShaftNode.NodeType.WhatDay, date.Copy(), studentNode);
        ...
        public void SkillUIUpdate()
        {
            ...
            {
                var control = Instantiate(playerSkillItemObject, playerSkillItemParent)
                    .GetComponent<PlayerSkillItemControl>();
                control.Init(gameManager);
                playerSkillItemControls.Add(control);
            }

            for (var i = 0; i < playerCourses.Count; i++) playerSkillItemControls[i].UpdateUI(playerCourses[i]);
        }
