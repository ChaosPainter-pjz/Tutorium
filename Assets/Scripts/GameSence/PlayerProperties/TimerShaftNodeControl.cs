...
namespace GameSence.PlayerProperties
{
    ...
    public class TimerShaftNodeControl : MonoBehaviour
    {
        ...
        {
            whatDay.text = "第" + timerShaftNode.date.Week + "周" +
                           timerShaftNode.date.ToString(Unit.Date.ToStringType.Week);
        }
        ...
        public void InspectNode()
        {
            ...
            {
                //寻找控制器中是否有与该节点相同的节点
                var isHave = studentNodeControls.Any(control => control.node == studentNode);
                //有则跳出
                if (isHave)
                    continue;
                //没有则增加
