...
namespace GameSence.World.Game.SelecStudent
{
    public class SelectStudentControl : MonoBehaviour
    {
        ...
        public void UpdateUI(List<string> gradeIds, int _number, GameObject _enterButton,
            WorldGameStartDelegate _delegate)
        {
            @delegate = _delegate;
            number = _number;
            ...
            for (var i = 0; i < StudentUnits.Count; i++)
            {
                ...
                {
                    var control = Instantiate(studentCartPrefab, studentCardParent)
                        .GetComponent<SelectStudentCardControl>();
        ...
        public void OnEnter()
        {
            var activeToggles = group.ActiveToggles();
            Toggle toggle = null;
            foreach (var to in activeToggles)
                if (to.isOn)
                    toggle = to;
            if (toggle is not null) student = toggle.GetComponent<SelectStudentCardControl>().studentUnit;
