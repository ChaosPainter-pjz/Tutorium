...
namespace GameSence.World.Game.PhotographyGame
{
    ...
    public class PhotographyGameControl : MonoBehaviour, IInit
    {
        ...
        public void Init(StudentUnit _studentUnit, int _number)
        {
            ...
            //初始化指针移动的速度、指针移动边界
            if (studentUnit != null) CountDifficulty(studentUnit);
            ...
            //开始播放音乐
            GameManager.GameManager.Instance.bgmAudioControl.StopPlayAll();
            GameManager.GameManager.Instance.bgmAudioControl.PlaySound("口哨");
            //开始“游戏”
            IsMove = true;
        }
        ...
        {
            var grade = _studentUnit.interestGrade.Find(x => x.gradeID == "19");
            ...
            if (IsMove)
            {
                if (isHaveTarget)
                {
                    backgroundRectTransform.anchoredPosition =
                        Vector2.SmoothDamp(backgroundRectTransform.anchoredPosition, target, ref currentVelocity, 1.8f);
                }
                else
                {
                    current = backgroundRectTransform.anchoredPosition;
                    var x = (current.x - Screen.width) / 2f;
                    var y = (current.y - Screen.height) / 2f;
                    target = new Vector2(Random.Range(-x, x), Random.Range(-y, y));
                    isHaveTarget = true;
                }

                if (Vector3.Distance(target, backgroundRectTransform.anchoredPosition) < 10f) isHaveTarget = false;
            }
        }
        ...
        private void MoveScrollbar()
        {
            if (IsMove)
            {
                ...
                {
                    scrollbar.value = Mathf.SmoothDamp(scrollbar.value, scrollbarTarget, ref currentTargetVelocity, 1f,
                        0.3f, Time.deltaTime * pointerSpeed);
        ...
        public void OnEnter()
        {
            if (IsMove == false)
                return;
            IsMove = false;
            var score = Mathf.Pow(1 - Mathf.Abs(scrollbar.value - 0.5f) * 2, 3) * 100;
            //加上标记物的分
            score += Marker(markersParent[backgroundNumber]);
            var achievement = new Achievement() { Value = score };
        ...
        {
            var score = 0;
            var transforms = parentObj.GetComponentsInChildren<RectTransform>();
            for (var i = 1; i < transforms.Length; i++)
            {
                ...
                if (screenPoint.x > 0 && screenPoint.x < Screen.width && screenPoint.y > 0 &&
                    screenPoint.y < Screen.height) score += int.Parse(transforms[i].name);
