...
namespace GameSence.World.Bookstore
{
    public class BookstoreManager : MonoInstance<BookstoreManager>
    {
        ...
        public int Money
        {
            get => MoneyManager.Instance.Money;
            set
            {
        ...
        {
            base.Awake();
            GameManager.GameManager.Instance.InitGameEvent += Init;
        }

        private void Init()
        {
            MoneyUpdate();
            storeGoodsList = GameManager.GameManager.Instance.StoreGoodsList;
            masterCourses = GameManager.GameManager.Instance.saveObject.SaveData.playerUnit.masterStudentCourse;
            foreach (var row in storeGoodsList.GetRowList())
            {
                var obj = Instantiate(commodityUnit, parent);
