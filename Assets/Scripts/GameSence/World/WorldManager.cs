...
using GameSence.World.KTV;
using GameSence.World.Market;
using GameSence.World.Supermarket;
using UnityEngine;
...
namespace GameSence.World
{
    public class WorldManager : MonoBehaviour
    {
        ...
        {
            HintManager.Instance.AddHint(new Hint.Hint("施工中", "该商户还在装修中，尚未开业"));
        }

        public void OnKtv()
        {
            if (gameManager.saveObject.SaveData.gameDate.year == gameManager.saveObject.SaveData.InitYear)
                HintManager.Instance.AddHint(new Hint.Hint("施工即将完成", "工作人员告诉你：该商户还在试营业中，将在明年第一周开业"));
            else
        ...
        public void OnSupermarket()
        {
            if (gameManager.saveObject.SaveData.gameDate.year == gameManager.saveObject.SaveData.InitYear &&
                ...
                HintManager.Instance.AddHint(new Hint.Hint("施工即将完成", "工作人员告诉你：该商户还在试营业中，将在今年下学期开业"));
            else
