...
namespace GameSence.StudentsProperties.分班
{
    ...
    public class DivideIntoClassesControl : MonoBehaviour
    {
        ...
        private void InitUI()
        {
            ...
            foreach (var control in cardControls) control.gameObject.SetActive(false);
            ...
            for (var i = 0; i < studentUnits.Count; i++) cardControls[i].Init(studentUnits[i]);
        }

        private void UpdateUI()
        {
            for (var i = 0; i < studentUnits.Count; i++) cardControls[i].UpdateUI();
        }

        public void DivideIntoClasses()
        {
            if (cardControls == null) OnEnable();
            foreach (var control in cardControls) control.ChooseSubject();
        }
    }
}