...
namespace GameSence.StudentsProperties
{
    public class StudentPropertiesControl : MonoBehaviour
    {
        ...
        {
            foreach (var obj in studentsPortrayals) obj.SetActive(obj.CompareTag(studentUnit.id));
        }
        ...
        private void SetBasic()
        {
            ...
            basicList[2].text =
                (GameManager.GameManager.Instance.saveObject.SaveData.gameDate - studentUnit.birthday).year + "岁";
            ...
            if (GameManager.GameManager.Instance.saveObject.SaveData.gameDate.year -
                DatetimeManager.Instance.InitYear == 0)
            {
            ...
            {
                basicList[6].text = "";
                foreach (var grade in studentUnit.NCEESelect) basicList[6].text += grade.name;
        ...
        private void SetInterestGrade()
        {
            ...
            {
                var entryControl = interestGrades.Find(x => x.grade.gradeID == grade.gradeID);
        ...
        {
            foreach (var control in friendControls) control.gameObject.SetActive(false);
