...
namespace GameSence.Schedule
{
    ...
    public class ParticularsPanelControl : MonoBehaviour
    {
        ...
        public void DescribeUIUpdate()
        {
            ...
            var classroomEfficiency = isClassroom ? gameManager.saveObject.SaveData.classroomEfficiency : 1.0f;
            //读取当前课程可以触发的老师的技能
            var targets = LoadPlayerCourse(courseRow);
            ...
            for (var i = 0; i < basicPropertiesBasic.Length; i++) //遍历刷新UI
            {
            ...
            string[] nameGrade =
            {
                "语文", "数学", "英语", "政治", "历史", "地理", "物理", "化学", "生物", "音乐", "表演", "舞蹈", "手工", "棋技", "种植", "摄影", "烹饪",
                "考古", "编程", "绘画", "运动"
            };
            ...
            for (var i = 0; i < nameGrade.Length; i++) //遍历刷新UI
            {
        ...
        private List<PlayerCourseLevelList.Row> LoadPlayerCourse(CourseList.Row inCourse)
        {
            ...
            foreach (var playerCourse in playerCourses)
            {
                ...
                PlayerCourseLevelList.Row GetTarget() //局部函数，通过playerCourse的数据来寻找合适的教师技能加成row条目
                {
                    ...
                    //找到符合该技能等级的升级数据
                    var allIdLevel = allId.FindAll(x => int.Parse(x.等级) == playerCourse.level);
        ...
        private void InitDescribe()
        {
            ...
            {
                var obj = Instantiate(courseAttributesEntryPrefab, courseAttributesEntryParent)
                    .GetComponent<CourseAttributesEntry>();
                obj.Init(gradeName, type);
                courseAttributesEntries.Add(obj);
            }

            courseAttributesEntries = new List<CourseAttributesEntry>();
            var unit = new StudentUnit();
            ...
            foreach (var str in basicPropertiesBasic)
