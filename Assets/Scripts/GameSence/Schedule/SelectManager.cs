...
public class SelectManager : MonoInstance<SelectManager>
{
    ...
    [Header("选择面板")]
    [SerializeField] private Transform mainGradeParent;
    ...
    public void OnPointerDown(CourseList.Row row,Vector3 position)
    {
        ...
        RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRec, Input.mousePosition,uiCamera, out mousePos);
        audioSourceUp.Play();
    }
    /// <summary>
    /// 指针被松开
    ...
    private void Update()
    {
        if (isDrag)
        {
            ...
            {
                Vector2 newVec;
                RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRec, Input.mousePosition, uiCamera, out newVec);
                //鼠标移动在画布空间的位置增量
    ...
    private void InitCard()
    {
        ...
        foreach (CourseList.Row row in gameManager.CourseList.GetRowList())
        {
            switch (row.Type)
            {
                case "M":
    ...
    private void CourseUIUpdate()
    {
        ...
        foreach (var masterCourse in masterStudentCourse)
        {
            ...
            if (otherControl)
            {
                otherControl.gameObject.SetActive(true);
            }
        }
    }
