...
public class StudentsManager : MonoInstance<StudentsManager>
{
    ...
    private void UIUpdate()
    {
        ...
        {
            studentCardControls[i].Init(studentUnits[i]);
        }
    }
    ...
    {
        StudentUnit unit = studentUnits.Find(x => x.id == id);

        if (unit == null)
        {
            StudentsList.Row row = gameManager.StudentsList.Find_id(id);
            unit = new StudentUnit
            {
                id = row.id,
                fullName = row.name,
                gender = row.gender switch {"男" => Gender.Man, "女" => Gender.Woman, _ => Gender.None},
                birthday = new Date(int.Parse(row.year)+gameManager.saveObject.SaveData.gameDate.year, int.Parse(row.semester), int.Parse(row.week), int.Parse(row.whatDay)),
            ...
            {
                unit.properties[i].score = row.Properties[i];
            }

            for (int i = 0; i < unit.mainGrade.Count; i++)
            {
                unit.mainGrade[i].score = row.MainGrade[i];
            }

            for (int i = 0; i < unit.interestGrade.Count; i++)
            {
                unit.interestGrade[i].score = row.InterestGrade[i];
            }
