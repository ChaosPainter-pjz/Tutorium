...
namespace GameSence.StoryLine
{
    ...
    public class StoryLineManager : MonoInstance<StoryLineManager>
    {
        ...
        {
            qualifiedPlot.Clear();
            foreach (var row in plotJudgmentList.GetRowList())
            {
                ...
                {
                    var studentUnit = saveData.studentUnits.Find(x => x.id == row.RoleID);
                       ...
                       )
                    {
                        ...
                        var currentValue = studentUnit.interpersonalRelationship.Find(x => x.id == row.FriendshipID)
                            .value;
            ...
            if (u != null)
            {
                ...
                if (u.ArticleID != "")
                {
                    ...
                    else
                    {
                        ...
                        HintManager.Instance.AddHint(new Hint.Hint($"得到{row.Name}*{u.ArticleNumber}", row.description));
                    }
                }
        ...
        private bool ChangeStudentProperty(string results)
        {
            ...
            if (sa != null)
            {
                var unit = saveData.studentUnits.Find(x => x.id == sa.StudentID);
                ...
                {
                    unit.schedule[int.Parse(sa.TimeBucket)] = new Unit.Schedule
                ...
                {
                    var relationship = unit.interpersonalRelationship.Find(x => x.id == sa.FriendID);
        ...
        private bool MakeHint(string results)
        {
            ...
            if (th != null)
            {
                if (th.isHint == "true") //是提示
                    HintManager.Instance.AddHint(new Hint.Hint(th.headline, th.text));
