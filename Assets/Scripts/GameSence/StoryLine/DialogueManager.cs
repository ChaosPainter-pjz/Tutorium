...
namespace GameSence.StoryLine
{
    public class DialogueManager : MonoInstance<DialogueManager>
    {
        ...
        [Header("面板绑定")] [SerializeField] private GameObject dialoguePanel;
        ...
        private void Basis(string text, string roleID, string rolePortrait)
        {
            ...
            var temporaryText = text.Replace("fullName", playerUnit.fullName);
        ...
        private void OnPanel()
        {
            ...
            if (plotID == "rename") StartDialogue();
        }
        ...
        public void OnClickPanel()
        {
            if (isPrint)
            {
                StopAllCoroutines();
                dialogueText.text = "";
                foreach (var s in showLog) dialogueText.text += s;
                isPrint = false;
                return;
            }
        ...
        {
            Sprite image = null;
            foreach (var sprite in resourceManager.scenes)
                if (sceneID == sprite.name)
                    image = sprite;
            foreach (var sprite in resourceManager.PhotoAlbum)
                if (sceneID == sprite.name)
                    image = sprite;
            scene.sprite = image == null ? resourceManager.scenes[0] : image;
            if (isSwitch) scenesAnimator.SetTrigger("switch");
        }
        ...
        private List<string> showLog = new();
        ...
        private void ShowLogInfo(string _log)
        {
            dialogueText.text = "";
            showLog.Clear();
            var splitToList = new List<List<string>>();
            //根据尖括号分开成文字和颜色
            var logChar = _log.Split('<', '>');
            //结果为{"龙虾的","color=#FFFFFFFF","Text","/color",""}
            foreach (var str in logChar)
            {
                var logcharArr = new List<string>();
                foreach (var cha in str) logcharArr.Add(cha.ToString());
                splitToList.Add(logcharArr);
            }

            //结果为{{龙,虾,的},{c,o,l,o,r,=,#,F,F,F,F,F,F,F,F},{T,e,x,t},{/,c,o,l,o,r},{}}
            for (var i = 0; i < logChar.Length; i++)
            {
                ...
                void Fwb(string strB)
                {
                    ...
                    var addColorString = new List<string>();
                    var addColorStingCache = new List<string>();

                    //拿到需要加颜色的字符
                    for (var k = 0; k < logChar[i - 1].Length; k++) addColorString.Add(logChar[i - 1][k].ToString());

                    //将字符加颜色
                    foreach (var t in addColorString)
                    {
                        if (t == "")
                            continue;
