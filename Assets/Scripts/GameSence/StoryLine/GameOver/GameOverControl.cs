...
namespace GameSence.StoryLine.GameOver
{
    public class GameOverControl : MonoBehaviour
    {
        [ContextMenuItem("end", "End")] [SerializeField]
        ...
        private void LoadStudent(StudentUnit unit)
        {
            ...
            var schoolRowList = enterHigherSchoolList.GetRowList().FindAll(row =>
            {
                if (int.TryParse(row.TotalPoints, out var thisTotalPoints)) return thisTotalPoints < totalPoints;
            ...
            {
                //判断该学生是否可以使用这个结局
                var ids = row.Student.Split(',');
                var isThisStudentOver = false;
            ...
            var maxGrade = unit.interestGrade[0]; //找出分最高的一个兴趣
            foreach (var grade in unit.interestGrade.Where(grade => grade.score > maxGrade.score)) maxGrade = grade;

            var schoolWorkRows = schoolWorkList.FindAll_TotalPoints(enterHigherSchoolRow.TotalPoints);
        ...
        private void LoadLongTimeStudent(StudentUnit unit)
        {
            ...
            var finds = new List<GameOverList.Row>();
            foreach (var row in gameOverList.GetRowList())
            {
                if (row.EndID == "") continue;
                var isEligibility = false;

                //判断该学生是否可以使用这个结局
                //判断学生ID要求
                var ids = row.StudentID.Split(',');
                if (ids.Any(str => str == unit.id)) isEligibility = true;

                if (row.StudentID != "all" && !isEligibility) continue;

                //基本属性要求
                isEligibility = true;
                for (var i = 0; i < unit.properties.Count; i++)
                {
                ...
                {
                    case "<":
                        if (unit.GoodAndEvil.score > (int.TryParse(row.善恶, out var value1) ? value1 : 0))
                            isEligibility = false;
                        break;
                    case ">":
                        if (unit.GoodAndEvil.score < (int.TryParse(row.善恶, out var value2) ? value2 : 0))
                            isEligibility = false;
                        break;
                }
            ...
            var rows0 = new List<GameOverList.Row>();
            var rows1 = new List<GameOverList.Row>();
            var rows2 = new List<GameOverList.Row>();
            var rows3 = new List<GameOverList.Row>();
            var rows4 = new List<GameOverList.Row>();
            var rows5 = new List<GameOverList.Row>();
            ...
            var target = finds[finds.Count - 1];
            if (rows5.Any())
            {
            ...
            if (gameManager.saveObject.OverSaveData.overUnits.All(un => un.overId != target.EndID))
                gameManager.saveObject.OverSaveData.overUnits.Add(new OverUnit(unit.fullName,
                    saveData.playerUnit.fullName, DateTime.Now, target.EndID));
        }
        ...
        public void End()
        {
            SaveManager.Scripts.SaveManager.SaveOverGame(gameManager.saveObject.OverSaveData);
            SaveManager.Scripts.SaveManager.ResetSaveFile(0);
