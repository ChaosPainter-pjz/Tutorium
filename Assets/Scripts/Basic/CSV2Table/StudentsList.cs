...
namespace Basic.CSV2Table
{
    public class StudentsList
    {
        public class Row : BasicRow
        {
            ...
            public string initialTrust;
            public string School;
            public string description;
        }

        public StudentsList(TextAsset csv)
        {
            Load(csv);
        }
        ...
        public void Load(TextAsset csv)
        {
            ...
            for (var i = 1; i < grid.Length; i++)
            {
                ...
                row.gender = grid[i][2];
                row.year = grid[i][3];
                row.semester = grid[i][4];
                row.week = grid[i][5];
                row.whatDay = grid[i][6];
                row.initialTrust = grid[i][7];
                row.School = grid[i][8];
                row.description = grid[i][9];
                row.气质 = grid[i][10];
                row.思维 = grid[i][11];
                row.口才 = grid[i][12];
                row.体质 = grid[i][13];
        ...
        public Row Find_initialTrust(string find)
        {
            return rowList.Find(x => x.initialTrust == find);
        }

        public List<Row> FindAll_initialTrust(string find)
        {
            return rowList.FindAll(x => x.initialTrust == find);
        }

        public Row Find_School(string find)
        {
            return rowList.Find(x => x.School == find);
        }

        public List<Row> FindAll_School(string find)
        {
            return rowList.FindAll(x => x.School == find);
        }
